...
function Model({ pitch, roll, rawYawInput }: { pitch: number; roll: number; rawYawInput: number }) {
  ...
  useFrame((_, deltaTime) => {
    ...
    const smoothingFactor = isReturningToCenter ? RETURN_SMOOTHING_FACTOR : MOVE_SMOOTHING_FACTOR;
...
const Dimensional3DAttitudeIndicator = memo(function Dimensional3DAttitudeIndicator({
  size,
  pitch,
  roll,
  rawYawInput,
  ...
  return (
    ...
    >
      ...
      >
        <text x={size * 0.1} y={size - size * 0.05} fill='oklch(0.985 0 0)' fontSize={size * 0.06}>
